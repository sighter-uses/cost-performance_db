...
/* ── 操作部 ── */
/* スティッキーの操作部。地の光と紋は画面いっぱいに広がっているので、本文カラム幅の
   不透明な箱を被せると黒い長方形が浮いて見える。背面を全幅に伸ばし、透過とぼかしで
   地を透かし、下端はグラデーションで溶かす。子要素は覆わないよう擬似要素に逃がす。 */
.controls{position:sticky;top:0;z-index:30;padding:.9rem 0 1.5rem;
